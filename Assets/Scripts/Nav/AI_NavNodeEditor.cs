...
public class NavNodeEditor : MonoBehaviour {
    ...
    private AI_NavNode navNode = null;
    private AI_NavNode activeNavNode = null;
    ...
    void OnScene(SceneView scene) {
        ...
        if (e.isMouse && (e.type == EventType.MouseMove || e.type == EventType.MouseDrag)) {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, 100, layerMask)) {
                ...
                spawnable = !(hitInfo.collider.gameObject.TryGetComponent<AI_NavNode>(out navNode));
                e.Use();
            }
            else {
    ...
    private void OnDrawGizmos() {
        ...
        foreach (AI_NavNode node in nodes) {
            foreach (AI_NavNode neighbors in node.neighbors) {
